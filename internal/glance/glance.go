...
func (a *application) handleWidgetRequest(w http.ResponseWriter, r *http.Request) {
	...
	// Check if widget implements handleRequest
	if hw, ok := widget.(interface{ handleRequest(w http.ResponseWriter, r *http.Request) }); ok {
		hw.handleRequest(w, r)
	} else {
		http.Error(w, "widget does not support requests", http.StatusNotImplemented)
	}
}
